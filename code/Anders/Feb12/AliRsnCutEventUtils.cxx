...
//
   ...
   if (fCheckIncompleteDAQ){
     if(fEvent->IsAOD()){
       AliAODEvent *aodEvt = (AliAODEvent*)(fEvent->GetRef());
       if(aodEvt && aodEvt->IsIncompleteDAQ()) accept=kFALSE;
     }else if(fEvent->IsESD()){
       AliESDEvent *esdEvt = (AliESDEvent*)(fEvent->GetRef());
       if(estEvt && esdEvt->IsIncompleteDAQ()) accept=kFALSE;
     }
   }
